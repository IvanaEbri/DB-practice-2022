...
SELECT * FROM tblusuarios LIMIT 10000;


#listar nombre y telefono de usuarios con telefonos que no sean blackberry

SELECT nombre, telefono FROM tblusuarios WHERE marca <> 'blackberry' LIMIT 10000;
...
select usuario AS 'login', activo FROM tblusuarios WHERE activo = 0;
select usuario AS 'login', activo FROM tblusuarios WHERE NOT activo;
...
SELECT MIN(saldo) AS 'Saldo minimo Hombres' FROM tblusuarios WHERE sexo = 'H';

SELECT Max(saldo) AS 'Saldo maximo Hombres' FROM tblusuarios WHERE sexo = 'H';
SELECT avg(saldo) AS 'Saldo promedio Hombres' FROM tblusuarios WHERE sexo = 'H';
SELECT saldo FROM tblusuarios WHERE sexo='h';


# Listar los números de teléfono con saldo mayor a 300
SELECT telefono AS 'Telefonos con saldo mayor a 300' FROM tblusuarios WHERE saldo > 300;

# Calcular la suma de los saldos de los usuarios de la compañia telefónica UNEFON
SELECT SUM(saldo) AS sumatoria_saldos FROM tblusuarios WHERE compañia = 'unefon';
...
SELECT usuario AS usuarios_SinSaldo_Inactivo FROM tblusuarios WHERE saldo = 0 OR activo = 0;

# Listar el login y teléfono de los usuarios con compañia telefónica IUSACELL o TELCEL
SELECT usuario AS login , telefono FROM tblusuarios 
